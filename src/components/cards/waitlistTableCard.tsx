...
async function getWalkins() {
    ...
    const records = await pb.collection(grabCollections('WalkIn')).getFullList({
        sort: '+DateTime',
        filter: `DateTime > "${startDay}" && DateTime < "${endDay}" && Table_Choice = "Table"`
    });
    return records;
};

export async function WaitlistTableCard() {
    try {
        const walkins = await getWalkins();
        return (
            <div className="text-9xl font-cursive overflow-auto no-scrollbar pb-20 pr-3 pt-1 w-1/2">
